...
Waifu2x::Waifu2x(std::string setFilename, int gpuid, bool _tta_mode, int num_threads)
{
...
namespace TaskManager {
	extern std::map<std::string, float> taskStatus;
	extern void updateTask(std::string , float);
};
int Waifu2x::process(const ncnn::Mat& inimage, ncnn::Mat& outimage, const std::string filename) const
{
    if (!vkdev)
    {
        // cpu only
    ...
    for (int yi = 0; yi < ytiles; yi++)
    {
        ...
        for (int xi = 0; xi < xtiles; xi++)
        {
			...
			//extern namespace TaskManager;

			
			TaskManager::updateTask(filename, (float)(yi * xtiles + xi) / (ytiles * xtiles) * 100);
